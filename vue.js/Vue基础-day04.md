...
#### 01-webpack-cli ：就是告诉你它有什么用：将浏览器不支持的源文件打包转换为浏览器支持的文件

使用命令：`webpack 输入文件路径 打包后文件路径`将一个文件打包成另外一个文件

#### 02-webpack-config：能够简化命令的输入。不用再指定源路径和目标路径

1. 配置webpack.config.js:文件不要随意
2. 运行`webpack`
...
#### 03-webpack-dev-server：能够自动打开浏览器，对于代码的修改能够自动刷新， 同时输入命令更爽
...
6. 配置script：`"dev": "webpack-dev-server --inline --hot --open --port 8090"` 在package.json文件中配置![1533263187684](C:\Users\ADMINI~1\AppData\Local\Temp\1533263187684.png)
7. `npm run dev`

#### 04-webpack-css:可以来解析css
...
#### 05-webpack-less&webpack-sass：可以创建和解析less和scss
...
#### 07-webpack-html：在html文件中自动引入js文件
...
```js
// 注意需要注释掉publicPath，不然会冲突  配置都是在webpack.config.js文件中
